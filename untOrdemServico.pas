...
begin
  ...
  begin
      Try
        vVlLiquido := StrToFloat(edtVlLiquido.Text);
...
begin
  ...
  begin
     ...
     if Trim(SQLOrdemServico.FieldByName('IN_STATUS').AsString) = 'INATIVO' then
     begin
...
begin
  ...
  try
    SQLOrdemServico.SQL.Text := 'select sum(VL_TOTAL) as VL_TOTALSERVICO from SERVICO_ORDSERV where CD_ORDSERV =' + edtCodigo.Text;
        SQLOrdemServico.Open;
    vVlLiquido := StrToFloat(SQLOrdemServico.FieldByName('VL_TOTALSERVICO').AsString);

    vVlLiquido := vVlLiquido - ((StrToFloat(edtDesconto.Text) / 100) * vVlLiquido);
