...
begin
  ...
  begin
    ...
    dm.sdsComandosSQL.ParamByName('DT_PEDIDO').AsDate := StrToDate(edtDtPedCompra.Text);
...
begin
  ...
  begin
     ...
     if Trim(SQLFornecedor.FieldByName('IN_STATUS').AsString) = 'INATIVO' then
     begin
       ShowMessage('Fornecedor inativo!');
       exit;
     end;

     if Trim(SQLFornecedor.FieldByName('CD_FORNECEDOR').AsString) = '' then
     begin
       ShowMessage('Fornecedor não encontrado!');
       edtCdFornecedor.SetFocus;
       exit;
